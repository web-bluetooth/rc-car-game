...
static void on_write(ble_lbs_t * p_lbs, ble_evt_t * p_ble_evt)
{
    ...
    if ((p_evt_write->handle == p_lbs->led_char_handles.value_handle) &&
        (p_evt_write->len != 0) &&
        (p_lbs->pin_write_handler != NULL))
    {
        p_lbs->pin_write_handler(p_lbs, p_evt_write->data);
    }
}
...
uint32_t ble_lbs_init(ble_lbs_t * p_lbs, const ble_lbs_init_t * p_lbs_init)
{
    ...
    p_lbs->conn_handle       = BLE_CONN_HANDLE_INVALID;
    p_lbs->pin_write_handler = p_lbs_init->pin_write_handler;
...
uint32_t ble_lbs_on_button_change(ble_lbs_t * p_lbs, uint8_t button_state, uint8_t byte_offset)
{
    ...
    params.p_len = &len;
    params.offset = byte_offset;
    
    return sd_ble_gatts_hvx(p_lbs->conn_handle, &params);
}

uint32_t ble_lbs_on_pin_change(ble_lbs_t * p_lbs, uint8_t pin_state, uint8_t byte_offset)
{
    ble_gatts_hvx_params_t params;
    uint16_t len = sizeof(pin_state);
    
    memset(&params, 0, sizeof(params));
    params.type = BLE_GATT_HVX_NOTIFICATION;
    params.handle = p_lbs->button_char_handles.value_handle;
    params.p_data = &pin_state;
    params.p_len = &len;
    params.offset = byte_offset;
